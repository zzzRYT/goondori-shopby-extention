...
상품심사(승인대기) 목록을 한 번에 스캔합니다. **등록 심사**(신규)는 MD가 설정한 규칙으로 평가해 위반 상품을 골라내고, **수정 심사**(이미 승인된 상품의 재심사)는 무엇이 바뀌었는지 변경(diff)을 보여 줍니다.

### 7-1. 스캔 준비

- 어드민에서 상품관리 → **상품심사(승인대기) 목록** 을 **활성 탭** 으로 엽니다.
- 목록 그리드(상품번호 표)가 렌더된 상태면 준비 완료입니다.
- 그리드를 못 찾으면 "상품심사 그리드를 찾지 못했어요" 안내가 뜹니다 — 목록을 연 뒤 다시 시작하세요.

### 7-2. 스캔 흐름

「▶ 스캔 시작」 을 누르면:

1. 페이지네이션을 **"200개 보기"** 로 바꾸고, 마지막 페이지까지 순회하며 상품번호를 수집합니다.
2. 수집한 상품마다 심사 팝업을 **숨김 탭** 에서 열어 파싱합니다. **동시 4개** 워커 풀(`SCAN_CONCURRENCY=4`)로 돌며, 하나가 끝나면 다음 상품을 곧바로 투입합니다.
3. 팝업 종류에 따라 갈라집니다 — **등록**은 켜진 규칙으로 평가, **수정**은 변경(diff) 추출.
4. 완료되는 상품부터 카드로 **스트리밍** 표시합니다(목록 순서가 아니라 완료 순서, 표시는 위반/변경 건수로 정렬).

> 동시 4개는 크롬 백그라운드 탭 스로틀링·어드민 레이트리밋을 고려한 보수적 기본값입니다. 더 올리면 렌더 타임아웃 실패가 늘어 4로 고정했습니다.

**진행 중 상태**

- 단계 라벨(목록 수집 중… / 상품 정보 수집 중…)과 진행률 `n / 전체`(완료 건수 기준)를 표시합니다.
- 카드는 완료되는 즉시 나타납니다 (전체 완료를 기다릴 필요 없음).
- 「⏸ 중단」 은 **신규 투입만** 멈춥니다 — 진행 중이던 최대 4건은 마무리되고, 그때까지 모은 결과는 유지됩니다.

### 7-3. 규칙 설정

규칙은 **등록 심사** 상품에만 적용됩니다 (수정 심사는 규칙과 무관하게 변경만 표시).

#### 기본 큐레이션 (기본 ON · 삭제 불가)

운영팀 큐레이션 체크리스트를 **기본 ON** 으로 제공합니다. 「규칙 설정」 → 「기본 큐레이션」 그룹에 항상 표시되며, 체크박스로 켜고 끌 수만 있고 삭제는 불가합니다 (지워도 다음 로드 때 복원).

| 큐레이션 | 위반 조건 | MD 편집 기준값 |
| --- | --- | --- |
| 역마진 경고 | 공급가 > 실판매가(즉시할인가) | — |
| 수수료 0% 경고 | 판매수수료 = 0% **그리고** 공급가도 비어 있음 (공급가가 채워져 있으면 통과 — 엑셀 업로드 케이스) | — |
| 할인율 이상치 | 할인율(=(판매가−즉시할인가)/판매가) ≥ 기준값 | 기본 70% |
| 대표이미지 누락 | 대표(썸네일) 이미지 없음 | — |
| 상세(상단/하단) 이미지 사용 | 상품 상세(상단)·(하단)에 이미지 있음 (앱에서 표현 안 되는 영역) | — |
| 상품명 글자수 초과 | 상품명 > 기준 글자수 | 기본 30자 |
| 서비스상품군 방지 | 배송구분 = `쇼핑몰 배송` (앱 미구현 추정 — 배송상품군은 `파트너사 배송`) | — |
| 재고 0개 경고 | 재고수량 = 0 | — |
| 전시카테고리 중복 | 전시카테고리 ≥ 2개 | — |
| 판매 종료 임박 | 판매기간 종료일이 오늘 기준 기준값 이내 | 기본 5일 |
| 가격 상한 초과 | 판매가·즉시할인가 > 기준값 (0 더 붙는 오기입 방지) | 기본 100만원 |

> 편집 가능한 기준값은 「기본 큐레이션」 그룹의 입력칸에서 바로 바꿉니다 (빈 값이면 기본값 적용).
> **미구현(후속)**: 옵션별 가격 편차, 품절/진열 큐레이션 — Shop API·진열 화면 통합 후 별도 단계로 추가 예정.

#### 시드 규칙 (기본 OFF)

자주 쓰는 규칙 5종을 **OFF** 상태로 미리 넣어 둡니다. 「규칙 설정」 → 체크박스로 켜서 사용합니다.

| 규칙 | 위반 조건 |
| --- | --- |
| 제조사명 필수 | 기본정보·제조사명 공란 |
| 제품모델명 필수 | 기본정보·제품모델명 공란 |
| 상품 중량 > 0 | 배송정보·상품 중량 0 이하 |
| 대표이미지 필수 | 대표 상품이미지 없음 |
| 상세 이미지 최소 1장 | 상세 이미지 1장 미만 |

#### 규칙 추가

「규칙 설정」 → 하단 추가 폼에서 **검사 유형** 을 고릅니다.

- **필수값 / 공란**: 섹션(기본정보 · 판매정보 · 배송정보 · 상품항목추가정보) → 항목 드롭다운 선택. (`공란`은 값이 있으면 위반 — 필수값의 반대)
- **기대값**: 비교 연산자(`=` · `≠` · `포함` · `>` · `≥` · `<` · `≤`)와 기대값 입력. 숫자는 `140,000원`→`140000` 자동 추출, `=`는 `상품수수료, 15%`↔`15%`도 같다고 판정.
- **계산 검사**: `역마진` / `할인율 상한` / `전시카테고리 개수 상한` / `글자수 상한` / `판매 종료 임박` / `가격 상한`.
- **이미지**: `대표이미지 필수` / `리스트이미지 필수` / `상세 이미지 최소 장수` / `상세 상단/하단 이미지 금지` / `허용 외 이미지 호스트 경고`.

항목은 오타 방지를 위해 자유 입력이 아니라 실제 심사 팝업 항목 카탈로그에서 고릅니다. 「규칙 추가」 를 누르면 **켜진 상태** 로 추가되고, 켜진 규칙은 상품마다 각각 평가되어 하나라도 걸리면 위반 카드로 표시됩니다.

> 설정한 규칙은 `chrome.storage.local` 에 **자동 저장** 되어 다음 스캔에도 유지됩니다.
...
- **세그먼트 필터**: `전체` / `등록` / `수정` 으로 종류를 추립니다.
- **위반만 보기**(기본 ON): 등록은 위반·실패만, **수정은 항상** 표시(검증된 상품의 변경이라 늘 검수 대상). 끄면 정상 등록(✅)도 함께 보입니다.
- **등록 카드**: `항목: 사유 (현재: 값)` 로 위반 내역을 표기합니다.
- **수정 카드**: `섹션 · 항목: 변경 전 → 변경 후` 로 변경 내역을 표기합니다.
- **카드 클릭**: 해당 상품 심사 팝업이 **새 활성 탭** 으로 열립니다 (파싱용 숨김 탭은 직후 자동으로 닫힘).
- 결과는 사이드패널 메모리에만 남습니다 — 패널을 닫으면 사라지므로 필요하면 재스캔하세요.

### 7-5. 경고 메시지

| 경고 | 의미 | 대응 |
| --- | --- | --- |
| 🔴 **세션 만료** | 어드민 로그인 만료로 스캔 즉시 중단 | 재로그인 후 재시작 |
| ⚠ **수집 건수 불일치** | 수집 수 ≠ "검색결과 총 N건" (일부 누락 가능) | 재스캔 권장 |
| ✖ **수집 실패 카드** | 특정 상품 팝업 파싱이 2회 시도 후 실패 | 어드민에서 직접 확인 |
