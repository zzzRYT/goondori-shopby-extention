...
type ApplyOptions = {
  ...
  // 저장 후 캡처한 메시지가 "실패"인지 판별. 기본은 키워드 매칭(실제 문구 미확보 → 보수적).
  isSaveError?: (message: string) => boolean;
};
...
const defaultIsSaveError = (message: string): boolean =>
...
export async function applyCategoryReorder(
  ...
  const isSaveError = options.isSaveError ?? defaultIsSaveError;
  ...
  try {
    for (let index = 0; index < request.steps.length; index += 1) {
      ...
      if (message !== null && isSaveError(message)) return abort(message);
