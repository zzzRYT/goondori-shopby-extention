...
전시 순서는 관리코드로 정해지는데, **관리코드는 중복이 허용되지 않습니다.** 따라서 두 카테고리의 순서를 맞바꾸려면 **비어 있는 임시 숫자를 경유** 해야 하고, **각 변경 사이에 「중복 확인」과 「저장」이 필수** 입니다. (그래야 다음 코드를 바꿀 때 중복이 생기지 않아 shopby 에서 오류 없이 적용됩니다.)

**예) `c_1` ↔ `c_2` 자리 바꾸기**

1. `c_1` → `c_3` 으로 변경 → **중복 확인** → **저장**
2. `c_2` → `c_1` 으로 변경 → **중복 확인** → **저장**
3. `c_3` → `c_2` 으로 변경 → **중복 확인** → **저장**
