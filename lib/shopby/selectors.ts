...
export type BannerFieldName = 'accountName' | 'width' | 'height' | 'landingUrl' | 'bannerName';

const BANNER_MAX_ACCOUNTS = 20;

export function bannerFieldKey(accountIndex: number, field: BannerFieldName): string {
  return `account${accountIndex}.${field}`;
}

function accountSelectors(i: number): FieldMap {
  return {
    [bannerFieldKey(i, 'accountName')]: `input[name="accounts.${i}.accountName"]`,
    [bannerFieldKey(i, 'width')]: `input[name="accounts.${i}.width"]`,
    [bannerFieldKey(i, 'height')]: `input[name="accounts.${i}.height"]`,
    [bannerFieldKey(i, 'landingUrl')]: `input[name="accounts.${i}.banners.0.landingUrlValue.landingUrl"]`,
    [bannerFieldKey(i, 'bannerName')]: `input[name="accounts.${i}.banners.0.bannerName"]`,
  };
}
