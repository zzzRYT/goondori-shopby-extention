...
describe('fetchSections', () => {
  ...
  it('sectionId가 있는 진열만 정규화해 반환한다', async () => {
    ...
    expect(result).toEqual([{ sectionNo: 1, sectionId: 'c_1_p_t_병부장', sectionName: '베스트' }]);
  });

  it('진열명이 비면 진열 ID를 라벨로 대체한다', async () => {
    stubJson({ sections: [{ sectionNo: 5, sectionId: 'c_2_s_b_43215615', sectionName: '   ' }] });

    const [entry] = await fetchSections('client');

    expect(entry.sectionName).toBe('c_2_s_b_43215615');
  });
