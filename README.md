...
6. 이미지: `대표이미지 필수` / `리스트이미지 필수` / `상세 이미지 최소 장수` / `허용 외 이미지 호스트 경고` 중 선택
...
> 심사 탭 추가로 인해 기존 섹션 번호가 이동했습니다. 전시카테고리 관련은 [8번 섹션](#8-전시카테고리-탭-사용법)을 참고하세요.
...
- 행의 「어드민에서 열기 ›」 를 누르면 위 어드민 트리에서 같은 카테고리가 **자동 선택** 됩니다. (→ [8-5](#8-5-행-클릭--어드민-트리에서-자동-선택편집))
- 코드를 **비워 둔** 상위 카테고리(미분류)와 **하위 카테고리** 는 미리보기 상위 목록에 나타나지 않습니다.

### 8-3. 환경 토글 (운영 c / 테스트 ct)
