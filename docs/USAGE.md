...
2. **검사 유형** 선택: `필수값` / `기대값` / `이미지`
3. 필수값·기대값: **섹션**(기본정보 · 판매정보 · 배송정보 · 상품항목추가정보) → **항목** 드롭다운 선택 — 오타 방지를 위해 자유 입력이 아니라 실제 심사 팝업 항목 카탈로그에서 고름
4. 기대값: **비교 연산자**(`=`, `≠`, `포함`, `>`, `≥`, `<`, `≤`)와 **기대값** 입력 — 숫자 비교는 `140,000원` → `140000`처럼 자동 추출되고, `=`는 `상품수수료, 15%` ↔ `15%`도 같다고 판정
5. 이미지: `대표이미지 필수` / `리스트이미지 필수` / `상세 이미지 최소 장수` / `허용 외 이미지 호스트 경고` 중 선택
