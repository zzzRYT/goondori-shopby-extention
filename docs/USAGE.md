...
- 어드민 탭: 상품심사 목록 페이지를 **활성 탭** 으로 오픈 (URL 경로: `/products/screen`)
- 목록 페이지는 **수정 가능 상태** (폼이 렌더된 상태)

### 7-2. 스캔 시작 → 목록 수집 흐름

사이드패널의 빨간 「스캔 시작」 버튼을 클릭하면:
...
| 규칙명 | 평가 내용 | 켜는 방법 |
| --- | --- | --- |
| `정가 필수` | 정가(`originalPrice`) 값이 있는가? | 규칙 우측 체크박스 활성화 |
| `판매가 필수` | 판매가(`salePrice`) 값이 있는가? | 규칙 우측 체크박스 활성화 |
| `상품이미지 개수` | 상품 이미지 개수가 설정값 이상인가? | 규칙 우측 입력란에 숫자 입력 후 활성화 |
| `상품설명 이미지 개수` | 상품설명(디테일) 이미지 개수가 설정값 이상인가? | 규칙 우측 입력란에 숫자 입력 후 활성화 |
| `규격 필수` | 규격 정보(`option`)가 있는가? | 규칙 우측 체크박스 활성화 |

#### 규칙 추가 (드롭다운)

1. 「규칙 추가」 버튼 클릭
2. 드롭다운에서 **필드명**(예: `SKU`, `구매가격`, `마진율` 등) 선택
3. **평가 조건** 설정 (필수값 유무 / 값 비교)
4. 「추가」 클릭

시드 규칙과 추가 규칙은 **모두 AND 조건** 으로 평가 (모든 규칙을 동시에 만족해야 통과)
...
- **카드 클릭**: 해당 상품의 심사 팝업이 **활성 탭** 으로 자동 이동 (숨김 탭에서 열려있던 팝업이 활성화됨)
- **위반 항목 표시**: 카드에 위반한 규칙명 일람 표시
...
| 경고 | 의미 | 대응 |
| --- | --- | --- |
| 🔴 **세션 만료됨** | 어드민 로그인이 만료되었음 (재인증 팝업 뜸) | 재로그인 후 스캔 재시작 |
| ⚠ **수집 건수 불일치** | 수집한 상품 수 ≠ 목록 검색결과 총 건수 | 네트워크 불안정 등의 원인 → 재스캔 권장 |
| ❌ **수집 실패** | 특정 상품의 팝업 파싱 실패 | 해당 상품을 어드민에서 직접 확인 후 재평가 |
